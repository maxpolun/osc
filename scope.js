...
function drawWithAxes(ctx, area, frequency, fn){
  var innerArea = drawAxes(ctx, area)
  drawFunction(ctx, innerArea, frequency, fn)
}
...
function drawFunction(ctx, area, frequency, fn){
  var w = area.rect.width
  var h = area.rect.height
  ...
  withContext(ctx, function(){
    ...
    for(var t = start; t < end; t += dt) {
      var x = w * (t/(end-start))
      ctx.lineTo(top.x + x, top.y + canvasY(fn(t*frequency), h))
    }
    ctx.stroke()
  })
}
...
Osc.FnDisplay.prototype.draw = function(osc){
  drawWithAxes(this.context, this.area, 1, osc.fn)
};
...
Osc.TimeDisplay.prototype.draw = function(osc, time){
  this.area.end = time
  drawWithAxes(this.context, this.area, osc.frequency, osc.fn)
}
